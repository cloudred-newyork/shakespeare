var shakes = (function ($) {
  ...
  function initRoleEvents() {
    $('#vizContent').on('click', '.infoContainer .role',function(){
      var role = $(this),
          ...
          sourceActor = role.parents('.infoContainer').attr('rel');
      role.parents('.infoContainer').remove();
      higlightItemsByRole(role.attr('rel'), role.text(),year, sourceActor);
    });
  }
  ...
  function initYears() {
    ...
    viewWidth = jQuery('body').innerWidth();
    viewHeight = jQuery(window).height();
    var a = Math.round((viewWidth * defaults.screenWidthToVizRatio)/2),
        b = Math.round((viewHeight * defaults.screenHeightToVizRatio)/2),
        ellipse,
        orbit,
        segmentLength;
  ...
  function higlightItemsByRole(prodID, role, exclude, sourceActor) {
      ...
      if (this.prodID == prodID) {
        ...
        $.each(this.cast, function(){ 
          if (matchRoles(this.Role, role) && ((prod.year != exclude) || (prod.year == exclude && sourceActor == this.actor_id)) ) { 
            ...
            if (otherActors[this.actor_id]) {
              otherActors[this.actor_id].roles.push(temp);
            } else {
              otherActors[this.actor_id] = {
                roles : [temp]
              }
            }
          }
    ...
    $.each(otherActors, function(key, val){
      filteredHilight(key, val);
    });
  }

  function filteredHilight(actorID, actorData) {
  ...
  function rollOverItem(e) {
    ...
    if (itemSelected) {
      if (UIMODE == 'actor' && itemsSelected.indexOf(parseInt(id)) > -1 ) {
  ...
  function itemClicked(e) {
    ...
    } else if (itemSelected && itemSelected == id) {
      resetSelection();
